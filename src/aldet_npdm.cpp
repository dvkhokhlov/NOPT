// aldet_casci_wrap density read-outs over NOPT's determinant CI: the per-state spin-summed
// 3-body moment (G3_calc_diag), assembled from the 3-body workers under the same OpenMP
// reduction aldet_data::G_calc uses; split from aldet_casci_wrap.cpp, which stays an adapter.
